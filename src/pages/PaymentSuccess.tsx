...
const PaymentSuccess = () => {
  ...
  useEffect(() => {
    const verify = async () => {
      if (!reference) {
        setErrorMsg("No payment reference found. Please contact support if you completed a payment.");
        setStage("error");
        return;
      }
      ...
      try {
        ...
        if (data.status === "success") {
          setInfo({ participantId: data.participantId, alreadyProcessed: data.alreadyProcessed });
          setStage("success");
        } else {
          setErrorMsg("Payment verification failed. Please contact our support team with your payment reference.");
          setStage("error");
        }
      } catch {
        setErrorMsg("A network error occurred while verifying your payment. Please try again or contact support.");
        setStage("error");
      }
    };
  ...
  const downloadUrl = info ? `${BACKEND_URL}/ticket-download/${info.participantId}` : "";
  ...
  return (
    <div className="psp">
      ...
      <AnimatePresence mode="wait">
        ...
        {/* VERIFYING */}
        {stage === "verifying" && (
          <motion.div key="verifying" className="psp__card"
            ...
            <p className="psp__sub">Please wait while we confirm your transaction…</p>
            <div className="psp__progress-bar"><div className="psp__progress-fill" /></div>
          </motion.div>
        )}

        {/* SUCCESS */}
        {stage === "success" && info && (
          <motion.div key="success" className="psp__card psp__card--success"
            initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}>

            {/* Check icon */}
            <motion.div className="psp__check"
              initial={{ scale: 0 }} animate={{ scale: 1 }}
              transition={{ delay: 0.15, type: "spring", stiffness: 300, damping: 20 }}>
              <svg viewBox="0 0 52 52" fill="none">
                <circle cx="26" cy="26" r="25" stroke="currentColor" strokeWidth="2" />
                <motion.path d="M14 27l8 8 16-16"
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
              ...
              <p className="psp__sub">
                ...
                {info.alreadyProcessed && " (This payment was already processed — your ticket is unchanged.)"}
              </p>
              ...
              <div className="psp__actions">
                ...
                <a href={ticketUrl} target="_blank" rel="noopener noreferrer" className="psp__btn psp__btn--secondary">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="psp__ticket-frame">
                <div className="psp__ticket-frame-label">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M2 9a3 3 0 010-6h20a3 3 0 010 6"/><path d="M2 15a3 3 0 000 6h20a3 3 0 000-6"/><line x1="12" y1="9" x2="12" y2="15"/>
                  </svg>
                  Your Conference Ticket
                </div>
                <iframe src={ticketUrl} title="Conference Ticket Preview" />
                <p className="psp__frame-hint">A copy has also been sent to your email address.</p>
              </div>

              <div className="psp__footer-nav">
                <Link to="/" className="psp__btn psp__btn--ghost">← Return to Homepage</Link>
              </div>
            </motion.div>
          </motion.div>
        )}
        ...
        {/* ERROR */}
        {stage === "error" && (
          <motion.div key="error" className="psp__card psp__card--error"
            initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="psp__contact-box">
              <p>Please contact our support team and provide the reference above:</p>
            ...
            <div className="psp__footer-nav">
              <Link to="/register" className="psp__btn psp__btn--primary">Try Again</Link>
              <Link to="/" className="psp__btn psp__btn--ghost">← Homepage</Link>
            </div>
          </motion.div>
        )}
